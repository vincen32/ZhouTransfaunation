...
metadata2 <- read.delim("Metadata_Zhou.txt", sep = "\t", header = T, quote = "", stringsAsFactors = F)
metadata2[1,]
metadata2[,1]
...
colnames(metadata)[7] <- "Days.Since.Start"
str(metadata)
...
Day28meta <- subset(metadata, metadata$Days.Since.Start == "28")
...
colors <- c("Black", "Blue", "Green", "Gray", "Red")

uwUF_meta <- uwUF$data$Vectors %>%
  select(SampleID, PC1, PC2, PC3) %>%
  inner_join(Day28meta, by = c("SampleID" = "SampleID"))

# Now we are going to make an ordination plot
ggplot(uwUF_meta, aes(x=PC1, y=PC2, color=DonationType)) +
  geom_point() + #alpha controls transparency and helps when points are overlapping
  theme_q2r() +
  xlab("PC1 (32.27%)") +
  ylab("PC2 (22.28%)") +
  scale_color_manual(values=c("Blue", "Black", "Green", "Gray", "Red"), name = "DonationType")

# Now we are going to make our code a little more re-usable
my_column <- "DonationType"
...
colnames(centroids)[1] <- "DonationType"
...
ggplot(uwUF_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=colors, name = my_column)
ggsave(paste0("output/Day28uwUF-ellipse_", my_column,".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) 
  #scale_color_manual(values=body_colors, name = my_column)
ggsave(paste0("output/uwUF-ellipse_", my_column,"-subject.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
