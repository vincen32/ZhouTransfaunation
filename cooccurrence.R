...
str(metadata)
colnames(metadata)[3] = "SampleName"

dataset <- merge(metadata, dataset, by.x = "SampleID", by.y = 0)
treatments<-as.vector(unique(dataset$DonationType))
datasetn<-dataset
datasetn[datasetn==0]<-NA

#i = 1

summary(metadata$RecipientID)
...
LL_ASVs<-subset(dataset, get(my_column)==treatments[1])

colnames(LL_ASVs[1:10])
head(final_results)
ggplot(LL_ASVs, aes(x = ASV15, y = ASV5)) +
  geom_point()
...
write.csv(subset(strong_results_taxa, trt=="LL"), "output/moving-pictures-strong-results-taxa-LL.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="HH"), "output/moving-pictures-strong-results-taxa-HH.csv", row.names = F)
