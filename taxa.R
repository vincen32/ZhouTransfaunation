...
metadata<-read_q2metadata("Metadata_Zhou.txt")
str(metadata)
...
colnames(metadata)[6] <- "Days.Since.Start"
...
Group8967 <- subset(metadata,metadata$RecipientID == "89" | metadata$RecipientID == "67")
Days.Since.Start.ord = factor(Group483481$Days.Since.Start, c("0", "1", "7", "28"))
...
metadata.filtered8967 = Group8967[row.names(Group8967) %in% colnames(physeq_otu_table),]
...
meta.physeq8967 = sample_data(metadata.filtered8967)

#We then merge these into an object of class phyloseq.

physeq_bar_plot8967 = phyloseq(OTU.physeq, tax.physeq, meta.physeq8967)
...
my_column <- "RecipientID"  #this is the metadata column that we will use in the taxa barplot
...
for(ml in my_level){
  ...
  taxa.summary <- physeq_bar_plot483481 %>%
  ...
  physeq_meta483481 <- merge(taxa.summary, physeq.taxa.max)
  
  
  physeq_meta_filtered483481 <- filter(physeq_meta483481, overall.max>abund_filter)
  #str(physeq_meta_filtered)
  
  physeq_meta_filtered483481$Days.Since.Start.ord = factor(physeq_meta_filtered483481$Days.Since.Start, c("0", "1", "7", "28"))
  
  # Plot 
  ggplot(physeq_meta_filtered483481, aes(x = get(my_column), y = Abundance.average, fill = get(ml))) + 
    facet_grid(~Days.Since.Start.ord) +
  ...
  ggsave(paste0("output/", ml, "BarPlot_483481", my_column, ".png"), height = 5, width = 4)
}
...
tax.physeq = tax_table(as.matrix(tax.clean))
meta.physeq = sample_data(metadata.filtered8967)
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ RecipientID)
...
alpha = 0.05
my_contrast = c("RecipientID", "483", "481") 
my_contrast = c("RecipientID", "89", "67") 
